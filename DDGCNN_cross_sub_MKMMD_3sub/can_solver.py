...
from DDGCNN_cross_sub_MKMMD_3sub.solver_utils import to_cuda, to_onehot, filter_samples, filter_class, split_samples_classwise,Sample,get_centers
from DDGCNN_cross_sub_MKMMD_3sub.cdd import CDD
from DDGCNN_cross_sub_MKMMD_3sub.clustering import Clustering
from DDGCNN_cross_sub_MKMMD_3sub.base_solver import BaseSolver
import DDGCNN_cross_sub_MKMMD_3sub.mmd_utils as mmd
...
class CANSolver(BaseSolver):
    ...
    def update_network(self):
        ...
        while not stop:
            ...
            for j in range(self.cdd_iter):
                ...
                source_mu_list = []
                for i in range(len(self.clustering_source_name)):
                    feature = self.extractor(source_data[i][0],source_data[i][1],source_data[i][2], source_data[i][3])
                    source_preds = self.classifier(feature)
                    gen_source_sample_list.append(feature)
                ...
                if len(self.filtered_classes) > 0:
                    tar_feature = self.extractor(tar_batch[0], tar_batch[1], tar_batch[2], tar_batch[3])
                    target_preds = self.classifier(tar_feature)
                    cdd_loss = self.cdd.forward(gen_source_sample_list, tar_feature, num_s_count, num_t_count)['cdd']
        ...
        while not stop:
            ...
            for i, (src_batch,tar_batch) in enumerate(zip(zip(*self.train_data[:len(self.clustering_source_name)]),self.train_data[len(self.clustering_source_name)])):
                ...
                for m in range(len(self.clustering_source_name)):
                    ...
                    cdd_loss += mmd.mix_rbf_mmd2(gen_source_sample_list[m], feature, sigma)
                cdd_loss *= self.opt.cdd_loss_weight
            ...
            self.source_test_accuracy, target_test_accuracy = self.test()
            self.logger.info(
                'Epoch :{}\t CE_loss :{:.3f}\t CDD_loss :{:.3f}\t Source KL :{:.3f}\t Target KL :{:.3f}\t Mu_bias :{:.3f}\t Sigma_bias :{:.3f}\t Target train acc :{:.2f}\t '
                'Target test_acc :{:.2f}\t'.format(epoch, ce_loss_iter / len(self.clustering_source_name), cdd_loss_iter, source_kl_iter, target_kl_iter,
                                                   mu_bias_loss_iter / len(self.clustering_source_name),sigma_bias_loss_iter / len(self.clustering_source_name),
                                                   target_accuracy_, target_test_accuracy))
            for i in range(len(self.clustering_source_name)):
                self.logger.info('Epoch :{}\t Source {} train acc :{:.2f}\t test_acc :{:.2f}'.format(epoch,self.clustering_source_name[i],source_accuracy_[i],self.source_test_accuracy[i]))
    ...
    def train_M3SDA(self):
        ...
        while not stop:
            ...
            for i, (src_batch, tar_batch) in enumerate(zip(zip(*self.train_data[:len(self.clustering_source_name)]),
                                                           self.train_data[len(self.clustering_source_name)])):
                loss_cls = 0
                source_feature = []
                weight_loss_list = []
                "Use classification loss and msda loss update Extractor and Classifier"
                for index, data in enumerate(src_batch):
                    ...
                    pred1 = self.classifier[self.clustering_source_name[index]]['c1'](feature)
                    pred2 = self.classifier[self.clustering_source_name[index]]['c2'](feature)
                    ...
                    weight_loss_list.append(loss_cls)
                    source_loss[self.clustering_source_name[index]]['Predictor_loss'] += loss_cls
                    source_num[index] += data1.shape[0]
                data = tar_batch
                data1, data2, data3, data4, y_t = data[0], data[1], data[2], data[3], data[4]
                total_num += data1.shape[0]
                tar_feature = self.extractor(data1, data2, data3, data4)
                # weight_list = [1., 1., 1., 1.]
                max_target_score = 1
                for index, data in enumerate(src_batch):
                    pred1 = self.classifier[self.clustering_source_name[index]]['c1'](tar_feature)
                    pred2 = self.classifier[self.clustering_source_name[index]]['c2'](tar_feature)
                    full_pred = (torch.softmax(pred1, dim=1) + torch.softmax(pred2, dim=1)) / 2
                    if isinstance(max_target_score, int):
                        max_target_score = torch.unsqueeze(torch.max(full_pred, dim=1).values, 1)
                    else:
                        max_target_score = torch.cat(
                            (max_target_score, torch.unsqueeze(torch.max(full_pred, dim=1).values, 1)), dim=1)
                class_tensor = torch.unsqueeze(torch.argmax(max_target_score, dim=1), 1)
                weight_list = []
                for index, data in enumerate(src_batch):
                    weight_label = (torch.ones((tar_feature.shape[0], 1)) * index).cuda()
                    weight_list.append((torch.sum(class_tensor == weight_label).item()) / tar_feature.shape[0])
                loss_cls = 0
                for index, data in enumerate(src_batch):
                    loss_cls += weight_loss_list[index] * weight_list[index]
                loss_m = 0.0008 * msda_regulizer(source_feature, tar_feature, 5, weight_list)
                # 3e 2e->50 2->62 5->64 8->64 10->64 50->63 40->62 30->62
                loss_1 = loss_cls + loss_m
                Step1_loss += loss_1
                ...
                "Use classification loss and target classify discrepancy loss update Predictor"
                tar_feature = self.extractor(data1, data2, data3, data4)
                d_loss = 0
                c_loss = 0
                for index, data in enumerate(src_batch):
                    ...
                    pred_c1 = self.classifier[self.clustering_source_name[index]]['c1'](tar_feature)
                    pred_c2 = self.classifier[self.clustering_source_name[index]]['c2'](tar_feature)
                    ...
                    for index_2, o_batch in enumerate(src_batch[index + 1:]):
                        pred_2_c1 = self.classifier[self.clustering_source_name[index_2 + index]]['c1'](tar_feature)
                        pred_2_c2 = self.classifier[self.clustering_source_name[index_2 + index]]['c2'](tar_feature)
                        combine2 = (torch.softmax(pred_2_c1, dim=1) + torch.softmax(pred_2_c2, dim=1)) / 2
                        dd_loss = discrepancy(combine1, combine2) * 0.1
                        d_loss += dd_loss
                    source_loss[self.clustering_source_name[index]]['Predictor_loss'] += (cc_loss - dd_loss) + loss_m
                ...
                target_num += tar_feature.shape[0]
                final_pred = 1
                for index_s in range(len(self.clustering_source_name)):
                    pred1 = self.classifier[self.clustering_source_name[index_s]]['c1'](tar_feature)
                    pred2 = self.classifier[self.clustering_source_name[index_s]]['c2'](tar_feature)
                    source_loss[self.clustering_source_name[index_s]]['Target train C1_loss'] += self.CELoss(pred1, y_t.long())
                    source_loss[self.clustering_source_name[index_s]]['Target train C2_loss'] += self.CELoss(pred2, y_t.long())
            ...
            for index, source in enumerate(self.clustering_source_name):
                ...
                for k in range(1, 3):
                    ...
                    source_loss[source]['C' + str(k) + '_Acc'] = source_loss[source]['C' + str(k) + '_Acc'] / source_num[index] * 100

            # self.logger.info(
            #     'Epoch :{}\t Train Extractor Loss: {:.5f}\t Train Step1 Loss: {:.5f}\t Train Step2 Loss: {:.5f}\t Train Step3 Loss: {:.5f}\t '.format(
            #         epoch,
            #         source_loss['Extractor_loss'], source_loss['Step1_loss'],
            #         source_loss['Step2_loss'], source_loss['Step3_loss']))
            for index, source in enumerate(self.clustering_source_name):
                self.logger.info(
                    'Source: {}\t Train Predictor loss: {:.5f}\t Train C1_loss: {:.5f}\t Train C2_loss: {:.5f}\t Train C1_Acc: {:.2f}\t Train C2_Acc: {:.2f}\t'.format(
                        source, source_loss[source]['Predictor_loss'], source_loss[source]['C1_loss'],
                        source_loss[source]['C2_loss'], source_loss[source]['C1_Acc'],
                        source_loss[source]['C2_Acc']))
        ...
        with torch.no_grad():
            ...
            for i, (src_batch,tar_batch) in enumerate(zip(zip(*self.test_data[:len(self.clustering_source_name)]),self.test_data[len(self.clustering_source_name)])):
                ...
                for index_s, data in enumerate(src_batch):
                    ...
                    source_feature = self.extractor(data1, data2, data3, data4)
                    pred1 = self.classifier[self.clustering_source_name[index_s]]['c1'](source_feature)
                    pred2 = self.classifier[self.clustering_source_name[index_s]]['c2'](source_feature)
