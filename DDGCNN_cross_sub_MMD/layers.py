...
class GraphConvolution(nn.Module):
    ...
    def forward(self, x, adj):
        out = torch.einsum('ijkl,kk->ijkl',[x.to('cuda'),adj.to('cuda')])
        self.conv = self.conv.to(self.device)
        if self.trans_class == 'linear':
            out = self.conv(torch.transpose(torch.squeeze(out),1,2))
            out = torch.unsqueeze(torch.transpose(out,1,2),3)
        else:
            out = self.conv(out)
        return out
...
def conv3x1(in_planes, out_planes, stride=(1,1), kernel_size=(3, 1),padding=(1,0)):
    """3x1 convolution with padding"""
    return nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, padding=padding, stride=stride, bias=False)
...
class GDCD(nn.Module):
    def __init__(self, inplanes, planes, stride, padding=(1,0), bias=True, dropout=0.2, kernel_size=(3, 1)):
        super(GDCD, self).__init__()

        self.conv = conv3x1(inplanes, planes, stride, kernel_size=kernel_size, padding=padding)
