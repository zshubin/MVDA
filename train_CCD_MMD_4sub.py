...
from DDGCNN_cross_sub_MMD.cdd_model import Extractor, Predictor
from DDGCNN_cross_sub_MMD.config import OptInit
import logging
from data_loader_cross_sub.prepare_data import *
from DDGCNN_cross_sub_MMD.subject_measure import subject_sim_measure
from DDGCNN_cross_sub_MMD.can_solver import CANSolver as Solver
import sys
...
def train(logger, sess, source_sub, tar_sub, rest_sub,record_file,top_coff):
    ...
    # Predictors = Predictor(opt.n_filters, opt.n_blocks, opt.class_num,).to(opt.device)
    Predictors = {}
    ...
    train_solver = Solver(Extractor_model, Predictors, record_file, top_coff= top_coff, dataloader=dataloaders, device=opt.device,
                          num_layers=opt.num_layers,kernel_num=opt.kernel_num,kernel_mul=opt.kernel_mul,
                          num_classes=opt.class_num,intra_only=opt.intra_only,clustering_source_name=para_source_sub,rest_sub_name=rest_sub,
...
if __name__ == '__main__':
    ...
    target_subject_list = ['s16', 's19', 's30', 's48', 's53']
    for sub in target_subject_list:
        subject_list.remove(sub)
    record_file = open('./record_ABMSDA_3fold_5source.txt','w')
    for i in range(len(target_subject_list)):
        ...
        record_file.write('target' + ' ' + target_subject + ' ' + 'source' + ' '
                          ...
                          + source_subject_list[4] + ' '
                          +'\n')
        rest_subject_list = top_sub[5:9]
