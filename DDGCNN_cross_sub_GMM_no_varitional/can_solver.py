...
from DDGCNN_cross_sub_GMM_no_varitional.solver_utils import to_cuda, to_onehot, filter_samples, filter_class, split_samples_classwise,Sample,get_centers
from DDGCNN_cross_sub_GMM_no_varitional.cdd import CDD
from DDGCNN_cross_sub_GMM_no_varitional.clustering import Clustering
from DDGCNN_cross_sub_GMM_no_varitional.base_solver import BaseSolver
import DDGCNN_cross_sub_GMM_no_varitional.mmd_utils as mmd
        ...
        while not stop:
            ...
            for i, (src_batch,tar_batch) in enumerate(zip(zip(*self.train_data[:len(self.clustering_source_name)]),self.train_data[len(self.clustering_source_name)])):
                ...
                for index, data in enumerate(src_batch):
                    ...
                    pred1 = self.classifier[self.clustering_source_name[index]]['c1'](source_feature)
                    pred2 = self.classifier[self.clustering_source_name[index]]['c2'](source_feature)
                    gen_source_sample_list.append(source_feature)
                ...
                gen_target_sample = self.extractor(
                ...
                gen_target_sample = self.extractor(data1, data2, data3, data4)
                d_loss = 0
                c_loss = 0
                for index, data in enumerate(src_batch):
                    ...
                    source_sample = self.extractor(data1, data2, data3, data4)
