...
from DDGCNN_cross_sub_MKMMD_3sub.cdd_model import Extractor, Predictor
from DDGCNN_cross_sub_MKMMD_3sub.config import OptInit
import logging
from data_loader_cross_sub.prepare_data import *
from DDGCNN_cross_sub_MKMMD_3sub.subject_measure import subject_sim_measure
from DDGCNN_cross_sub_MKMMD_3sub.can_solver import CANSolver as Solver
import sys
...
def train(logger, sess, source_sub, tar_sub, rest_sub,record_file,top_coff):
    ...
    Extractor_model = Extractor([opt.batch_size, opt.in_channels, opt.eeg_channel], opt.k_adj, opt.n_filters,
                                opt.dropout,
                                bias=opt.bias, norm=opt.norm, act=opt.act, trans_class=opt.trans_class,
...
if __name__ == '__main__':
    ...
    record_file = open('./record_CAN_0.8.txt','w')
