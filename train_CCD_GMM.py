...
from DDGCNN_cross_sub_GMM.cdd_model import Extractor, Predictor
from DDGCNN_cross_sub_GMM.config import OptInit
import logging
from data_loader_cross_sub.prepare_data import *
from DDGCNN_cross_sub_GMM.subject_measure import subject_sim_measure
import sys
from DDGCNN_cross_sub_GMM.can_solver import CANSolver as Solver
...
def train(logger, sess, source_sub, tar_sub, rest_sub,record_file,top_coff):
    ...
    # train_solver = Solver(Extractor_model, Predictor_model, record_file, top_coff= top_coff, dataloader=dataloaders, device=opt.device,
...
if __name__ == '__main__':
    ...
    for i in range(len(target_subject_list)):
        ...
        record_file = open('./record_source_analysis_{}.txt'.format(target_subject),'w')
        cluster_iter = 1000
        stop_epsion = 1e-10
        num_classes = 4
        top_number = 20
        ...
        for j in range(5):
            source_subject_list = top_sub[j*source_num:(j+1)*source_num] ##0-2, 2-4, 4-6, 6-8
            record_file.write('target' + ' ' + target_subject + ' ' + 'source' + ' '
                              + top_sub[j*source_num] + ' '
                              + top_sub[j*source_num+1] + ' '
                              + '\n')
