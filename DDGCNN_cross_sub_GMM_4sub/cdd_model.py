...
from DDGCNN_cross_sub_GMM_4sub.layers import GraphConvolution,Linear,GDCD,grad_reverse
from DDGCNN_cross_sub_GMM_4sub.utils import normalize_A, generate_cheby_adj, randomedge_drop
from DDGCNN_cross_sub_GMM_4sub.solver_utils import Sample
...
class Extractor(nn.Module):
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        mu = self.conv3(result)
        mu = self.norm3(mu).to(self.device)
        mu = mu.flatten(1, 3)
        mu = torch.squeeze(mu)

        # mu_bias = self.conv_bias3(result)
        # mu_bias = self.norm4(mu_bias).to(self.device)
        # mu_bias = self.act_bias3(mu_bias)
        # mu_bias= self.droplayer_bias2(mu_bias)
        # mu_bias = mu_bias.flatten(1, 3)
        # mu_bias = torch.squeeze(mu_bias)

        # sigma = self.conv4(result)
        # sigma = self.norm4(sigma).to(self.device)
        # sigma = sigma.flatten(1,3)
        # sigma = torch.squeeze(sigma)

        # sigma_bias = self.conv_bias4(result)
        # sigma_bias = self.norm6(sigma_bias).to(self.device)
        # sigma_bias = self.act_bias4(sigma_bias)
        # sigma_bias = self.droplayer_bias4(sigma_bias)
        # sigma_bias = sigma_bias.flatten(1, 3)
        # sigma_bias = torch.squeeze(sigma_bias)

        # sample = Sample(mu, sigma, self.device)
        # bias_sample = Sample(mu+mu_bias, sigma+sigma_bias, self.device)

        # return mu, sigma, mu_bias, sigma_bias, sample, bias_sample
