...
import os
from DDGCNN_cross_sub_GMM_4sub.solver_utils import mean_accuracy, accuracy, adjust_learning_rate_exp, adjust_learning_rate_inv, adjust_learning_rate_plateau
...
class BaseSolver:
    ...
    def update_lr(self):
        ...
        if self.opt.lr_schedule == 'exp':
            adjust_learning_rate_exp(self.base_lr,self.extractor_optimizer, iters,decay_rate=self.opt.lr_decay_rate,decay_step=self.opt.decay_step)
            adjust_learning_rate_exp(self.base_lr, self.classifier_optimizer,
                                     iters,
