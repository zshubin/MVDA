...
from DDGCNN_cross_sub_GMM_4sub.solver_utils import to_cuda, to_onehot, filter_samples, filter_class, split_samples_classwise,Sample,get_centers
from DDGCNN_cross_sub_GMM_4sub.cdd import CDD
from DDGCNN_cross_sub_GMM_4sub.clustering import Clustering
from DDGCNN_cross_sub_GMM_4sub.base_solver import BaseSolver
import DDGCNN_cross_sub_GMM_4sub.mmd_utils as mmd
...
def forward_distribution_loss(source_sigma,source_mu,target_sigma,target_mu):
    target_sigma_ = target_sigma + 1e-3
    target_sigma_ = torch.where(torch.isnan(target_sigma_), torch.full_like(target_sigma_, 1e-3), target_sigma_)
    source_sigma_ = []
    for i in range(len(source_mu)):
        tt = source_sigma[i]+1e-3
        tt = torch.where(torch.isnan(tt), torch.full_like(tt, 1e-3), tt)
        source_sigma_.append(tt)
    dis_loss = 0.
    for i in range(len(source_mu)):
        dis_loss += (target_sigma_-source_sigma_[i])+(torch.exp(source_sigma_[i]**2)+(source_mu[i]-target_mu)**2)/(2*torch.exp(target_sigma_)**2)-0.5
    dis_loss /= len(source_sigma)
    return dis_loss

def backward_distribution_loss(source_sigma,source_mu,target_sigma,target_mu):
    target_sigma_ = target_sigma + 1e-3
    target_sigma_ = torch.where(torch.isnan(target_sigma_), torch.full_like(target_sigma_, 1e-3), target_sigma_)
    source_sigma_ = []
    for i in range(len(source_mu)):
    ...
    dis_loss = 0.
    for i in range(len(source_mu)):
        dis_loss += (source_sigma_[i]-target_sigma_)+(torch.exp(target_sigma_**2)+(target_mu-source_mu[i])**2)/(2*torch.exp(source_sigma_[i])**2)-0.5
    dis_loss /= len(source_sigma)
    return dis_loss
...
class CANSolver(BaseSolver):
    ...
    def complete_training(self):
        if self.loop >= self.opt.max_loop or len(self.filtered_classes)==0:
    ...
    def update_network(self):
        ...
        while not stop:
            ...
            self.logger.info('Epoch :{}\t CE_loss :{:.3f}\t CDD_loss :{:.3f}\t  Target train acc :{:.2f}\t Target test_acc :{:.2f}\t'.format(
                epoch, ce_loss_iter/4, cdd_loss_iter, target_accuracy_, target_test_accuracy))
            for i in range(len(self.clustering_source_name)):
    ...
    #                         'iters': self.iters,
    #                         'extractor_state_dict': self.extractor.state_dict(),
    #                         'classifier_S1_c1_state_dict': self.classifier[self.clustering_source_name[0]]['c1'].state_dict(),
    #                         'classifier_S1_c2_state_dict': self.classifier[self.clustering_source_name[0]]['c2'].state_dict(),
    #                         'classifier_S2_c1_state_dict': self.classifier[self.clustering_source_name[1]]['c1'].state_dict(),
    #                         'classifier_S2_c2_state_dict': self.classifier[self.clustering_source_name[1]]['c2'].state_dict(),
    #                         'classifier_S3_c1_state_dict': self.classifier[self.clustering_source_name[2]]['c1'].state_dict(),
    #                         'classifier_S3_c2_state_dict': self.classifier[self.clustering_source_name[2]]['c2'].state_dict(),
    #                         'classifier_S4_c1_state_dict': self.classifier[self.clustering_source_name[3]]['c1'].state_dict(),
    #                         'classifier_S4_c2_state_dict': self.classifier[self.clustering_source_name[3]]['c2'].state_dict(),
    #                         }, filename)
    #         print('Best target test acc:', self.best_acc)
    #         update_iters += 1
    #         self.iters += 1
