...
import os
from DDGCNN_cross_sub_GMM.solver_utils import mean_accuracy, accuracy, adjust_learning_rate_exp, adjust_learning_rate_inv, adjust_learning_rate_plateau
...
class BaseSolver:
    ...
    def test(self):
        ...
        source_total_num = 0
        target_total_num = 0
        for i, (src_batch, tar_batch) in enumerate(zip(zip(*self.test_data[:len(self.clustering_source_name)]),
                                                       self.test_data[len(self.clustering_source_name)])):
            for index, data in enumerate(src_batch):
                data1, data2, data3, data4, y, path = data[0], data[1], data[2], data[3], data[4], data[5]
                data1, data2, data3, data4, y = data1.to(self.device), data2.to(self.device), data3.to(
                    self.device), data4.to(
                    self.device), y.to(self.device)

                # mu, sigma, mu_bias, sigma_bias, sample, bias_sample = self.extractor(data1, data2, data3, data4)
                mu, sigma, sample = self.extractor(data1, data2, data3, data4)
                source_preds = self.classifier(sample)
            ...
            # mu, sigma, mu_bias, sigma_bias, sample, bias_sample = self.extractor(data1, data2, data3, data4)
            mu, sigma, sample = self.extractor(data1, data2, data3, data4)
            target_preds = self.classifier(sample)
    ...
    def rest_test(self):
        ...
        for i, rest_batch in enumerate(self.rest_data):
            for index, data in enumerate(rest_batch):
                    ...
                    mu, sigma, mu_bias, sigma_bias, sample, bias_sample = self.extractor(data1, data2, data3, data4)
                    rest_preds = self.classifier(sample)
